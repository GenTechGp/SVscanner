...
# Directories
classifierDir=/directflow/KCCGGenometechTemp/projects/jasyip/SVtoolkit/SVclassifier
...
splitRM=${splitDir}/RMtmp

# Input Files
refFASTA=/directflow/KCCGGenometechTemp/projects/andmar/human_genome_pipeline/References/hg38/hg38noAlt.fa
svVCF=/directflow/KCCGGenometechTemp/projects/jasyip/samples/HG002_22/HG002_chr22.vcf.gz
strBED=/directflow/KCCGGenometechTemp/projects/jasyip/references/STRchive-disease-loci.bed
...
# Create the split directory (remove it if it already exists)
rm -rf "$splitDir"
mkdir -p $splitDir
mkdir -p $splitRM
...
chmod +r $splitDir/*
chmod +w $splitDir/*
find "$splitDir" -name "${sample}.[0-9]*" | parallel -j $NTHREADS "
    output_fasta=\"{}.fa\"
    id_file=\"{}_id.tab\"
...
python3 ${ANNOTATE} -id $IDfile -trf $TRFfile -rm $RMfile -vcf $annotatedVCF -trf_tsv $trfTSV -rm_tsv $rmTSV -sv_vcf $svVCF -min $minIntersect -mr $minCoverage -div $interval -strchive $strBED
